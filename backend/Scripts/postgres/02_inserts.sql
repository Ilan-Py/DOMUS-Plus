-- ============================================================
-- DOMUS+ | Datos de prueba (Postgres) — traducido de Scripts/02_inserts.sql
-- SOLO para entorno de desarrollo/testing. NO ejecutar en producción.
-- Requiere haber ejecutado postgres/01_schema.sql primero.
-- Passwords en texto plano aquí; en la app se guardan hasheados.
--
-- Sin cambios de sintaxis respecto al original — INSERT ... VALUES
-- multi-fila es idéntico en MySQL y Postgres, no había nada
-- MySQL-específico en este script (no ON DUPLICATE KEY, no funciones
-- propietarias). Se quita únicamente el `USE domus_db;` (no existe en
-- Postgres — la base ya queda seleccionada por la connection string).
-- ============================================================
...
-- ------------------------------------------------------------
-- GRUPOS FAMILIARES + MEMBRESÍA (uno por usuario, cada uno dueño del suyo)
-- El esquema consolidado ya no tiene grupo_familiar.usuario_id (eso era
-- pre-03_grupo_compartido.sql) — la pertenencia siempre pasa por
-- grupo_miembro, incluso en el seed baseline.
-- ------------------------------------------------------------
INSERT INTO grupo_familiar (nombre) VALUES
('Familia Pitashny'),
('Familia Di Pasquale'),
('Familia Acevedo');
...
-- ------------------------------------------------------------
-- RECORDATORIOS
-- Esquema consolidado: recordatorio.grupo_id NOT NULL (compartido por
-- grupo) + creado_por (ex usuario_id). El grupo de cada fila es el mismo
-- que el del creador (grupo_id == usuario_id para este seed, 1:1 dueño).
-- ------------------------------------------------------------
INSERT INTO recordatorio (grupo_id, creado_por, vacuna_id, tratamiento_id, tipo, fecha_hora, descripcion, activo) VALUES
(1, 1, 2,    NULL, 'vacuna',      '2025-09-01 09:00:00', 'Refuerzo doble viral de Alduin',      TRUE),
(1, 1, NULL, 1,    'medicacion',  '2025-06-15 08:00:00', 'Control mensual Enalapril Gotenks',   TRUE),
(2, 2, NULL, 2,    'medicacion',  '2025-06-20 22:00:00', 'Insulina nocturna Maximiliano',        TRUE),
(1, 1, 3,    NULL, 'vacuna',      '2025-11-15 10:00:00', 'Antirrábica anual Rufus',             TRUE),
(3, 3, NULL, NULL, 'control',     '2025-07-01 11:00:00', 'Turno veterinario Bruno — revisión',  TRUE);
